...
static void Difficulty_DrawChoices(int selection, int y, u8 textSpeed);
...
static int Difficulty_ProcessInput(int selection);
...
} static const sItemFunctions[MENUITEM_COUNT] =
{
    ...
    [MENUITEM_TRANSITION] = {TextSpeed_DrawChoices, FourOptions_ProcessInput},
    [MENUITEM_DIFFICULTY] = {Difficulty_DrawChoices, Difficulty_ProcessInput},
...
static int BattleStyle_ProcessInput(int selection)
{
    // On hard mode, 'SET' is forced
    if (sOptions->sel[MENUITEM_DIFFICULTY] < DIFFICULTY_HARD)
...
static int Difficulty_ProcessInput(int selection)
{
    ...
    selection = FourOptions_ProcessInput(selection);

    // Force 'Set' on hard difficulty
    if (selection >= DIFFICULTY_HARD && sOptions->sel[MENUITEM_BATTLESTYLE] == 0)
    {
        sOptions->sel[MENUITEM_BATTLESTYLE] = 1;
        // Show change if Battle Style is visible
        id = sOptions->visibleCursor - (MENUITEM_DIFFICULTY - MENUITEM_BATTLESTYLE);
        if (id >= 0 && id <= 6)
            BattleStyle_DrawChoices(1, id * Y_DIFF, 0xFF);
    }
    return selection;
}
...
static void Difficulty_DrawChoices(int selection, int y, u8 textSpeed)
{
    FourOptions_DrawChoices(sDifficultyStrings, selection, y, textSpeed);
}
